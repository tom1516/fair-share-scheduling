...
/* The process pointed to by 'mp' has forked.  Create a child process. */
  ...
  int child_nr, s;
  phys_clicks prog_clicks, childbase;
  ...
  */
  ...
  if ( (childbase = alloc_mem(prog_clicks)) == NO_MEM) return(ENOMEM);

  /* Create a copy of the parent's core image for the child. */
  child_abs = (phys_bytes) childbase << CLICK_SHIFT;
  ...
  *rmc = *rmp;			/* copy parent's process slot to child's */
   ...
   */
  if (!(rmc->mp_flags & SEPARATE)) rmc->mp_seg[T].mem_phys = childbase;
  rmc->mp_seg[D].mem_phys = childbase;
  ...
  sys_fork(who, child_nr,rmc->mp_procgrp);
