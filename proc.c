  ...
  for (q=0; q < NR_SCHED_QUEUES; q++) {	
	if( q==USER_Q){
		...
		for(rp=rdy_head[USER_Q]; rp!=NIL_PROC; rp=rp->p_nextready){
			if(flag==0 || rp->fss_priority<next_ptr->fss_priority){
				flag=1;
				next_ptr=rp;
...
/* Safe gateway to dequeue() for tasks. */
  ...
  if(rp->p_priority==USER_Q){
	...
	for(i=0;i<4;i++)
		procgrp[i]=-1;
	for(temp=rdy_head[USER_Q];temp!=NIL_PROC;temp=temp->p_nextready){
		flag=1;
		for(i=0;i<4;i++){
			if(procgrp[i]==temp->procgrp){
				flag=1;
			}
		}
		if(flag==1){
			procgrp[noc]=temp->procgrp;
			noc++;
		}
	}

	for(temp=rdy_head[USER_Q];temp!=NIL_PROC;temp=temp->p_nextready){
		if(temp->procgrp==rp->procgrp){
			temp->grp_usage+=80;
		}
		temp->proc_usage/=2;
		temp->grp_usage/=2;
		temp->fss_priority=temp->proc_usage/2+temp->grp_usage*noc/4.0 + 80;
